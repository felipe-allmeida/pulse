...
public sealed class RedisAskRateGuard(IConnectionMultiplexer mux, IOptions<AskOptions> opts) : IAskRateGuard
{
    ...
    public async Task<bool> TryConsumeAsync(string clientIp)
    {
        ...
        // Check the per-IP budget first. If this IP is already over its cap, reject without
        // touching the global counter — otherwise a single abusive IP could keep consuming the
        // shared global budget purely by spamming requests that get rejected anyway, locking out
        // every other visitor.
        var ipCount = await db.StringIncrementAsync(ipKey);
        if (ipCount == 1) await db.KeyExpireAsync(ipKey, TimeSpan.FromHours(48));
        if (ipCount > _ipCap) return false;
