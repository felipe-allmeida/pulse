...
if (redirectTo) {
  ...
  //
  // Ahead of the mount rather than inside it: `loadClarity` only appends an
  // `async` script, so it costs nothing on the critical path, and starting it
  // before `router.load()` resolves gives the recorder the whole startup
  // window rather than only what follows the swap.
