...
/**
 * The Ask panel's body: the message list, the composer, and `streamAsk`.
 *
 * Split out of `ask-widget.tsx` (Task 7, fix round 1) so the trigger and the
 * `<Sheet>` shell can stay eager — present at first paint, present in
 * prerendered HTML — while this half, the part that only matters once the
 * sheet is actually open, loads as its own chunk. `ask-widget.tsx` only
 * renders this behind `<Suspense>` once the shared store's `isOpen` (or a
 * chip's `pendingQuestion`) has been true at least once, so the chunk is
 * fetched on first open, not at page mount.
 *
 * Rendered as a child of the eager `<Sheet>` in `ask-widget.tsx` — Radix's
 * Dialog context reaches through the Suspense boundary the same as any other
 * React context, so `SheetContent` here is still wired to that `<Sheet>`'s
 * open state and close handling.
 */
export function AskPanel() {
  ...
  // The sheet closing — trigger, a chip re-opening elsewhere, Escape, an
  // overlay click, anything that flips the shared store's `isOpen` — cancels
  // an in-flight stream. This used to run from AskWidget's own
  // `onOpenChange`; it lives here now because the abort controller (and the
  // fetch it controls) is part of this lazily-loaded half, while the Sheet
  // root that owns `onOpenChange` stays in the eager shell.
  useEffect(() => {
  ...
  //
  // This effect is also the one that has to survive this component mounting
  // *after* a chip already set `isOpen`/`pendingQuestion` on the store,
  // because the chunk this file ships in hadn't loaded yet — it reads the
  // store's current values on its very first run, same as any other render.
  useEffect(() => {
