...
describe('useVisitFeed', () => {
  ...
  it('seeds at most SEED_COUNT events, keeping the most recent ones', () => {
    ...
    // batchOf numbers cities oldest-first, so the newest twelve are 8..19,
    // and the feed's top entry is the newest of all.
    expect(events[0].kind === 'visit' && events[0].city).toBe('City 19');
    expect(events[11].kind === 'visit' && events[11].city).toBe('City 8');
  });
  ...
  it('does not re-emit the first batch entries it chose not to seed', () => {
    ...
    expect(useEventStore.getState().events).toHaveLength(12);
  });
